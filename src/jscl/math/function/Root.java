...
public class Root extends Function implements Algebraic {
	public static final Parser parser=RootParser.parser;
	Generic subscript;
	...
	public Generic derivative(Variable variable) {
		if(compareTo(variable)==0) return JSCLInteger.valueOf(1);
			...
			UnivariatePolynomial p=UnivariatePolynomial.valueOf(parameter,this);
			UnivariatePolynomial q=(UnivariatePolynomial)p.derivative().multiply(t.expressionValue()).add(UnivariatePolynomial.valueOf(a,this));
			UnivariatePolynomial r=UnivariatePolynomial.valueOf(p.resultant(q),t);
	...
	static Generic cubic(Generic parameter[], int subscript) {
		...
		for(int i=0;i<y.length;i++) {
			y[i]=new Pow(
				...
				new Inv(JSCLInteger.valueOf(3)).evalsimp()
			).evalsimp();
		}
		switch(subscript) {
		case 0:
		...
		case 1:
			return new Frac(
				a.add(y[0]).add(Constant.half.negate().add(Constant.i.multiply(new Sqrt(JSCLInteger.valueOf(3)).expressionValue().multiply(Constant.half))).multiply(y[0])).subtract(Constant.half.negate().add(Constant.i.multiply(new Sqrt(JSCLInteger.valueOf(3)).expressionValue().multiply(Constant.half))).multiply(y[1])),
				JSCLInteger.valueOf(3)
			).evalsimp().negate();
		default:
			return new Frac(
				a.subtract(Constant.half.negate().add(Constant.i.multiply(new Sqrt(JSCLInteger.valueOf(3)).expressionValue().multiply(Constant.half))).multiply(y[0])).add(y[1]).add(Constant.half.negate().add(Constant.i.multiply(new Sqrt(JSCLInteger.valueOf(3)).expressionValue().multiply(Constant.half))).multiply(y[1])),
				JSCLInteger.valueOf(3)
			).evalsimp().negate();
		}
	}
	...
	static Generic quartic(Generic parameter[], int subscript) {
		...
		case 0:
			return new Frac(
				a.subtract(y[0]).subtract(y[1]).subtract(y[2]),
				JSCLInteger.valueOf(4)
			).evalsimp().negate();
		case 1:
			return new Frac(
				a.subtract(y[0]).add(y[1]).add(y[2]),
				JSCLInteger.valueOf(4)
			).evalsimp().negate();
		case 2:
			return new Frac(
				a.add(y[0]).subtract(y[1]).add(y[2]),
				JSCLInteger.valueOf(4)
			).evalsimp().negate();
		default:
			return new Frac(
				a.add(y[0]).add(y[1]).subtract(y[2]),
				JSCLInteger.valueOf(4)
			).evalsimp().negate();
		}
	}
	...
	public Generic evalnum() {
		return NumericWrapper.root(subscript.integerValue().intValue(),(NumericWrapper[])parameter);
	}
	...
	public String toMathML(Object data) {
		...
		if(exponent==1) {
			...
			buffer.append(1,subscript.toMathML(null));
			buffer.append("</msub>\n");
		} else {
...
class RootParser extends Parser {
	...
	public Object parse(String str, int pos[]) throws ParseException {
		...
		try {
			subscript=(Generic)Constant.subscriptParser.parse(str,pos);
		} catch (ParseException e) {
			pos[0]=pos0;
			throw e;
		}
